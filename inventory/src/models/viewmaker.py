...
class Double_Viewmaker(Viewmaker):
    '''Viewmaker network that stochastically maps a multichannel 2D input to an output of the same size.'''
    def __init__(self, num_in_channels=3, num_out_channels=3 * 2, distortion_budget=0.05, activation='relu',  
                clamp=True, frequency_domain=False, downsample_to=False, num_res_blocks=3):
        '''Initialize the Viewmaker network.

        Args:
            num_in_channels: Number of channels in the input (e.g. 1 for speech, 3 for images)
                Input will have shape [batch_size, num_channels, height, width]
            num_out_channels: Number of output channels for the viewmaker (e.g. 1 for speech, 3 for images, 3 * 2 for 2 noise pattern)
            distortion_budget: Distortion budget of the viewmaker (epsilon, in the paper).
                Controls how strong the perturbations can be.
            activation: The activation function used in the network ('relu' and 'leaky_relu' currently supported)
            clamp: Whether to clamp the outputs to [0, 1] (useful to ensure output is, e.g., a valid image)
            ...
            num_res_blocks: Number of residual blocks to use in the network.
        '''
        super().__init__(num_channels=num_in_channels, distortion_budget=distortion_budget, activation=activation,  
                clamp=clamp, frequency_domain=frequency_domain, downsample_to=downsample_to, num_res_blocks=num_res_blocks)
        
        self.num_out_channels = num_out_channels
        
        #Overide last layer of viewmaker
        # Upsampling Layers for the last layer
        self.deconv3 = ConvLayer(32, self.num_out_channels, kernel_size=9, stride=1)
    ...
    def forward(self, x):
        ...
        # Additive perturbation with 3 channel
        result1 = x + delta[:,:3,:,:]
        result2 = x + delta[:,3:,:,:]
        
        if self.clamp:
            result1 = torch.clamp(result1, 0, 1.0)
            result2 = torch.clamp(result2, 0, 1.0)

        return result1, result2

class SPN_Viewmaker(Viewmaker):
    '''Viewmaker network that stochastically maps a multichannel 2D input to an output of the same size.'''
    def __init__(self, num_channels=3, activation='relu', clamp=True, num_res_blocks=3):
        '''Initialize the Viewmaker network.
        '''
        ...
        for m in self.modules():
            ...
            if hasattr(m, 'weight') and classname.find('Conv') != -1:
                torch.nn.init.normal_(m.weight.data, mean=0, std=1e-4)
    ...
    def basic_net(self, x, num_res_blocks=5, bound_multiplier=1):
        ...
        y = self.add_noise_channel(x, bound_multiplier=bound_multiplier)
        y = self.act(self.in1(self.conv1(y)))
        y = self.act(self.in2(self.conv2(y)))
        y = self.act(self.in3(self.conv3(y)))
        ...
        y = torch.cat([rot1, rot2, rot2, rot3, rot4, rot1],1) * torch.tensor([1,1,0,1,1,0],device=y.device)

        y = y.view(-1, 2, 3)
        
        grid = F.affine_grid(y, x.size())
    ...
    def forward(self, x):
        y = x
        y = self.basic_net(y, self.num_res_blocks, bound_multiplier=1)
        if self.clamp:
